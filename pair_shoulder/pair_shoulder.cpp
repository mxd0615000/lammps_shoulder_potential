  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        if (eflag) evdwl = epsilon[itype][jtype] * pow((sigma[itype][jtype]*rinv) ,epxn[itype][jtype])+ 0.5 * epsilon[itype][jtype] * (1 - tanhij) - offset[itype][jtype] ;evdwl *= factor_ij;
        if (evflag) ev_tally(i, j, nlocal, newton_pair, evdwl, 0.0, fpair, delx, dely, delz);
      }
    }
  }
...
void PairShoulder::allocate()
{
  ...
  memory->create(expn, n + 1, n + 1, "pair:expn");
