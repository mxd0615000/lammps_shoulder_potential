...
PairShoulder::PairShoulder(LAMMPS *lmp) : Pair(lmp)
{
  respa_enable = 1;
  writedata = 1;
  single_enable = 1;
...
// void PairShoulder::compute(int eflag, int vflag)
// {
...
void PairShoulder::settings(int narg, char **arg)
{
...
void PairShoulder::coeff(int narg, char **arg)
{
  ...
  double epxn_one = utils::numeric(FLERR, arg[4], false, lmp);
  double k0_one = utils::numeric(FLERR, arg[5], false, lmp);
  double sigmas_one = utils::numeric(FLERR, arg[6], false, lmp);
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        forceij = epsilon[itype][jtype] *epxn[itype][jtype]* pow((sigma[itype][jtype]*rinv), epxn[itype][jtype]+1) *sigma[itype][jtype] * rinv * rinv + 0.5 * epsilon[itype][jtype] * k0[itype][jtype] * (1 - tanhij * tanhij);
        ...
        if (eflag) evdwl = epsilon[itype][jtype] * pow((sigma[itype][jtype]*rinv) ,epxn[itype][jtype])+ 0.5 * epsilon[itype][jtype] * (1 - tanhij) - offset[itype][jtype] ;evdwl *= factor_lj;
        if (evflag) ev_tally(i, j, nlocal, newton_pair, evdwl, 0.0, fpair, delx, dely, delz);
      }
    }
  }
...
// double PairShoulder::init_one(int i, int j)
// {
//   if (setflag[i][j] == 0) {
...
double PairShoulder::init_one(int i, int j)
{
  ...
  if (offset_flag) {
    ...
    offset[i][j] =
        epsilon[i][j] * pow((sigma[i][j]/cut[i][j]),epxn[i][j]) + 0.5*epsilon[i][j] * (1- tanh(k0[i][j]*(cut[i][j]-sigmas[i][j])));
  } else
    offset[i][j] = 0.0;
...
void PairShoulder::write_restart(FILE *fp)
{
...
void PairShoulder::write_restart_settings(FILE *fp)
{
...
void PairShoulder::read_restart(FILE *fp)
{
...
void PairShoulder::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
void PairShoulder::write_data(FILE *fp)
{
...
void PairShoulder::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
    for (int j = i; j <= atom->ntypes; j++)
      fprintf(fp, "%d %d %g %g %g %g %g %g\n", i, j, epsilon[i][j], sigma[i][j], epxn[i][j], k0[i][j], sigmas[i][j], cut[i][j]);
}

/* ---------------------------------------------------------------------- */

double PairShoulder::single(int i, int j, int itype, int jtype, double rsq, double factor_coul,
                            double factor_lj, double &fforce)
{
  ...
  philj = epsilon[itype][jtype] * pow((sigma[itype][jtype]*rinv) ,epxn[itype][jtype])+ 0.5 * epsilon[itype][jtype] * (1 - tanhij);
...
void *PairShoulder::extract(const char *str, int &dim)
{
  dim = 2;
